...
class AnimalBySearchParams extends Component {
  constructor(props) {
    super(props);
  ...
  componentDidMount() {
    this.cageList();
    this.animalList(this.props.match.params.searchParam);
  }
  compare(a,b){
    if (a < b) {
      return -1;
  ...
  animalList(searchParam) {
    this.setState({isLoading: true});
    fetch('http://localhost:3000/animals?'+searchParam)
    .then(result=>result.json())
  .then(items=>this.setState({animals: items,isLoading:false}))
  }

  getAnimalsByfilter(filter,value) {
    let url = 'http://localhost:3000/animals?'+this.props.match.params.searchParam
    if(!url.includes(filter)){
      this.setState({isLoading: true});
    fetch(url+ '&'+filter+"="+value)
    .then(result=>result.json())
  ...
  render() {
    ...
    if(this.state.filter){
      buttonClearFilter = (<div><br/><Button onClick={() => this.clearFilter()} color="warning" size="sm" id="stat"><FontAwesomeIcon icon={Icons.faFilter}/> Effacer les filtres</Button> <br/><br/></div>);
    }
    if(this.state.showId){
      animalList = this.state.animals.map((item, i) => (
        ...
        <Link to={{ pathname: '/searchAnimal', state: { from: '/animals'} }}><Button outline color="primary"><FontAwesomeIcon  icon={Icons.faSearch}/> Rechercher</Button></Link>
        {' '}        <Link to={{ pathname: '/animals', state: { from: '/searchAnimal'} }}><Button outline color="danger"><FontAwesomeIcon  icon={Icons.faTimesCircle}/> Annuler</Button></Link>
        </Col>);
        ...
        column = (  <Table hover striped>
          ...
          <th > 
          <Link to={{ pathname: '/animals', state: { from: '/searchAnimal'} }}> <Button outline color="warning" size="sm"><FontAwesomeIcon icon={Icons.faFilter}/> Effacer la recherche</Button></Link>
{' '}
             {buttonClearFilter}{' '}
...
export default  AnimalBySearchParams;
